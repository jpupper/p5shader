...
uniform float flushx ;
uniform float flushy ; 
...
{   
    ...
    float mscx = mapr(scalef,0.0,30.0) ; 
   // float mscy =  mapr(scaley,0.0,30.0) ;
    float mspeedx = mapr(speedx,-.1,0.1);
    float mspeedy = mapr(speedy,-0.1,0.1);
    float mflushx = mapr(flushx,0.0,5.0);
    float mflushy = mapr(flushy,0.0,5.0);



   // color += ridgedMF(vec2(uv.x*3.0,uv.y*3.0)*ridgedMF(vec2(uv.x*1.0,uv.y*1.0))*sin(uv.x*30.)*sin(uv.y*30.));
     color += ridgedMF(vec2(uv.x*mflushx,uv.y*mflushy)*ridgedMF(vec2(uv.x*1.0+mspeedx,uv.y*1.0+mspeedy))
                *sin(uv.x*mscx+mspeedx*time)*sin(uv.y*mscx+mspeedy*time));
