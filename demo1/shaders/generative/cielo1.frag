...
void main(void){   
    ...
    vec2 uv = gl_FragCoord.xy/resolution.xy;
         uv = vTexCoord;
    float e = fbm(uv*4.+fbm(uv*8.,time+100.),time+100.);
        //  e = rxr(uv*0.2);
            
        //  e = sin(uv.x*freq+sin(uv.y*20.+time))*sin(uv.y*freq)*.5+.5;
          
          
          //e = sin(uv.y*freq+sin(uv.x*15.+time)*.2+sin(uv.x*50.)*.2)*.5+.5;
          //e = smoothstep(0.0,0.8,e);
          //e+=sin(e*20.)*.2;
         // e = smoothstep(0.4,0.8,e);

       //  e = sin(uv.x*2.)*.5+.5;
    vec3 c1= vec3(r1,g1,b1);
    vec3 c2 = vec3(r2,g2,b2);
    vec3 fin = mix(vec3(r1,g1,b1),vec3(r2,g2,b2),e);

         fin = mix(fin,vec3(1.),0.7);
    //fin = dib2;
